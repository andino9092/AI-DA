...
- **Timers and reminders:** `set_timer` / `list_timers` / `cancel_timer`, parsed locally ("set a timer for 10 minutes", "remind me in 20 minutes to take out the laundry", "how much time is left"). Saved to `timers.json`, so they survive a restart; one that went off while AI-DA was closed is announced on the next start if it's under 10 minutes late. Going off plays an alarm chime, is spoken when Aida isn't busy, and shows a Windows notification.
- **Files and folders:** `open_folder` (Downloads, Documents, Desktop, Pictures, Music, Videos, home, or any folder by name) and `open_file` (recent files from Windows' Recent items, then Desktop/Documents/Downloads up to 4 levels, skipping node_modules/.git/AppData). Two equally good matches → Aida asks which; programs and scripts ask before running.
- **Steam games:** Steam's Start-menu shortcuts are `steam://rungameid/…` links, which couldn't be launched through the Apps folder; link ids are now opened as links. The Steam libraries (from `libraryfolders.vdf`) are also scanned for games without a shortcut (27 found on Andy's PC).
